...
# price_bma_JZS <- bas.lm(price_log ~ ., data = df_pricemod, prior = "JZS", 
#                     alpha = 1, 
#                     n.models = 2^15,
#                     modelprior = uniform(),
...
#                     method = "MCMC")
#Save Model
#saveRDS(price_bma_JZS, "~/Homework/STAT 723/Case-Study-2-team-3/Ezinne/saved_Models/price_bma_JZS.rds")
# price_bma_JZS <- readRDS("~/Homework/STAT 723/Case-Study-2-team-3/Ezinne/saved_Models/price_bma_JZS.rds")
# #print(price_bma_JZS )
...
pop_bma2 <- bas.lm(popularity_log ~ ., data = df_popmod_no_outliers, prior = "JZS", 
                    alpha = 1, 
                    n.models = 2^15,
                    initprobs = "marg-eplogp",
                    method = "MCMC",
...
#Save Model
#saveRDS(pop_bma2, "~/Homework/STAT 723/Case-Study-2-team-3/Ezinne/saved_Models/pop_bma_JZS2.rds")
#Reading in saved model
pop_bma_JZS2 <- readRDS("~/Homework/STAT 723/Case-Study-2-team-3/Ezinne/saved_Models/pop_bma_JZS2.rds")
#print(pop_bma_JZS2)
...
residuals_price_bma <- df_pricemod_no_outliers$price_log - predicted_price_bma$fit
...
df_pricemod_no_outliers %>% 
  ...
  ggplot(aes(x = longitude, y = latitude, color = residuals_price_bma)) + geom_point() + theme_dark() + scale_color_gradient2(midpoint = 4) 
...
residuals_pop_bma <- df_popmod_no_outliers$popularity_log - predicted_pop_bma$fit
...
df_popmod_no_outliers %>% 
  ...
  ggplot(aes(x = longitude, y = latitude)) + geom_point(aes(color = residuals_pop_bma_ind))

#Saving posterior mean and interval estimates 
pop_var_names <- c("Intercept", "Neighbourhood_group:Manhattan", "Neighbourhood_group:Queens", "Neighbourhood_group:Staten Island", "Neighbourhood_group:Bronx","latitude","longitude", "Room_type:Entire home/apt", "Room_type:Shared room", "number of reviews", "last_review", "reviews_per_month", "calculated_host_listings_count", "availability_365", "name_listing_sentiment", "proximity_attraction","name_host_freq","name_host_special:True" ,"name_listing_length", "type_stay:Long","proximity_metro") 
...
rf.residuals.price <- df_pricemod_no_outliers$price_log - predict(price_rf) 
...
rf.residuals.pop <- df_popmod_no_outliers$popularity_log - predict(pop_rf) 
