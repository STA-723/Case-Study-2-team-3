...
d_clean_price_rf <- d_clean%>% 
  select(-id,-name,-host_id, -host_name, -popularity, -price, -neighbourhood, -last_review, -days_since_last,-availability_365, -dist_closest_metro, -popularity_log) %>% 
   # short term listing
  ...
  rename(listing_sentiment= name_listings_sentiment) 

d_clean_pop_rf <- d_clean%>% 
  select(-id,-name,-host_id, -host_name, -price, -neighbourhood, -last_review, -days_since_last,-availability_365,-dist_closest_metro, -price_log, -popularity) %>% 
  # short term listing
  filter(minimum_nights < 29) %>%
...
d_clean_price_bma<- d_clean%>% 
  select(-id,-name,-host_id, -host_name, -popularity, -price, -neighbourhood, -last_review,-days_since_last, -availability_365, -dist_closest_metro, -popularity_log, -longitude, -latitude) %>% 
  # short term listing
  filter(minimum_nights < 29) %>%
  select(price_log, everything()) %>% 
  filter(!is.na(price_log)) %>% 
...
d_clean_pop_bma <- d_clean%>% 
  select(-id,-name,-host_id, -host_name, -price, -neighbourhood,-days_since_last, -last_review,-availability_365, -dist_closest_metro, -price_log, -popularity, -longitude, -latitude) %>% 
    # short term listing
  filter(minimum_nights < 29) %>%
  select(popularity_log, everything()) %>% 
  filter(!is.na(popularity_log)) %>% 
