...
compute_proximity_5_closest_attractions <- function(longitude, latitude){
  d_attraction %>% 
...
d <- d %>%
  ...
  # Proximity to 5 closest attrications
  mutate(proximity_attraction = list(longitude, latitude) %>% pmap_dbl(compute_proximity_5_closest_attractions))
```
